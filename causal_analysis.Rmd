...
```{r}
st(data_started, group = 'condition', vars = relevant_features, group.test = list(
...
W.train <- ifelse(train$condition=='loss', 1, 0)
X.train <- train[relevant_features]

Y.test <- test$converted
W.test <- ifelse(test$condition=='loss', 1, 0)
...
cate_p1 <- ggplot(cate.test, aes(x = predictions, fill = cate_direction)) +
  ...
  scale_x_continuous(labels = scales::label_number(accuracy = 0.01, scale = 100)) +
...
ggplot() +
  ...
  scale_y_continuous(labels = scales::label_number(accuracy = 0.01, scale = 100)) +
...
gate_p1 <- ggplot(data = cate_sign_df) +
  ...
  geom_text(aes(x = 1, y = -0.065), label = round(cate_sign_df[2,"estimate"]*100,3)) +
  geom_text(aes(x = 2, y = 0.065), label = round(cate_sign_df[1,"estimate"]*100,3)) +
  scale_y_continuous(labels = scales::label_number(accuracy = 0.01, scale = 100)) +
  ylab("Group ATE") +
...
ggsave("SHAP - Beeswarm Conversion.pdf", width = 10, height = 8)
...
gate_p3 <- ggplot(data = gender_df) +
  ...
  geom_text(aes(x = 1, y = 0.02), label = round(gender_df[2,"estimate"]*100,3)) +
  geom_text(aes(x = 2, y = 0.02), label = round(gender_df[1,"estimate"]*100,3)) +
  ylab("Group ATE") +
  xlab("Gender") +
  theme(legend.position = "none") +
...
gate_p5 <- ggplot(data = policy_df) +
  ...
  geom_text(aes(x = 1, y = 0.025), label = round(policy_df[1,"estimate"]*100,3)) + 
  geom_text(aes(x = 2, y = 0.025), label = round(policy_df[2,"estimate"]*100,3)) + 
  ylab("Group ATE") +
...
gate_p5 <- ggplot(data = mng_df) +
  ...
  geom_text(aes(x = 1, y = 0.025), label = round(mng_df[1,"estimate"]*100,3)) + 
  geom_text(aes(x = 2, y = 0.025), label = round(mng_df[2,"estimate"]*100,3)) + 
  ylab("Group ATE") +
...
ggsave("RATE - Saving Amount.pdf", width = 10, height = 8)
```
...
ggsave("CATE_GATE - Saving Amount.png", width = 10, height = 8)
ggsave("CATE_GATE - Saving Amount.png", 
       width = 10, 
       height = 8)
...
sv_importance(shap_values, kind = "bee")
ggsave("SHAP - Beeswarm Saving Amount.pdf", width = 10, height = 8)
...
ggsave("GATE by Gender - Saving Amount.pdf", width = 10, height = 8)
```
...
ggsave("RATE by Gender - Saving Amount.pdf", width = 10, height = 8)
```
...
ggsave("GATE by Contract Valuation - Saving Amount.pdf", width = 10, height = 8)
```
...
ggsave("RATE by Contract Valuation - Saving Amount.pdf", width = 10, height = 8)
```
...
ggsave("GATE by Mng Skills - Saving Amount.pdf", width = 10, height = 8)
```
...
ggsave("RATE by Mng Skills - Saving Amount.pdf", width = 10, height = 8)
```
...
```{r}
probit_converted_2021 <- glm(converted ~ condition,
                        family=binomial(link='probit'), 
                        data=data_started %>% filter(year==2021))

probit_converted_2022 <- glm(converted ~ condition,
                        family=binomial(link='probit'), 
...
```{r}
ols_amount_2021 <- lm(log(paid_amount) ~ condition,
                 data=data_finished %>% filter(year==2021))

ols_amount_2022 <- lm(log(paid_amount) ~ condition,
